...
18. **`unstable_after` requires config flag** — Must add `experimental: { after: true }` to `next.config.js` before importing. Missing this causes a runtime error at import time.
