...
import { TherapistSwapQueue } from '@/components/swaps/TherapistSwapQueue'
  ...
  if (profile.role === 'therapist') {
    ...
    return (
      <div className="max-w-2xl mx-auto py-6 px-4 space-y-4">
        <h1 className="text-lg font-semibold text-slate-900">My pending swaps</h1>
        <p className="text-sm text-slate-500">
          These requests are waiting for a manager to approve or reject.
        </p>
        {swaps.length === 0 ? (
          <p className="text-sm text-slate-500">No pending swap requests.</p>
        ) : (
          <TherapistSwapQueue swaps={swaps} />
        )}
      </div>
    )
  }
